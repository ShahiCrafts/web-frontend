...
const ManageCommunities = () => {
    ...
    return (
        <div className="px-6 py-5 bg-white min-h-screen font-sans text-gray-800">
                    ...
                    <AnimatePresence>
                        {myCommunities.map((community) => {
                            ...
                            return (
                                <motion.div
                                    key={community._id}
                                    className="relative bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all flex flex-col overflow-hidden p-5"
                                    initial={{ opacity: 0, y: 15 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    exit={{ opacity: 0, y: -15 }}
                                    transition={{ duration: 0.4 }}
                                    ...
                                    <p className="text-sm text-gray-600 mt-2 flex-grow"> {/* flex-grow to push footer down */}
                                        ...
                                        {hasLongDescription && (
                                            <span className="inline-block"> {/* Use inline-block to keep button with text, but allow layout control */}
                                                ...
                                                <button
                                                    onClick={() => toggleDescription(community._id)}
            ...
            <AnimatePresence>
                {isCreateModalOpen && <CreateCommunityModal onClose={handleCloseModal} />}
            </AnimatePresence>
        </div>
    );
};
