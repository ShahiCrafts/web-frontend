...
// provinceData, getColor, getRange, and Legend components remain unchanged...
const provinceData = [
...
// --- TOOLTIP COMPONENT REFACTORED ---
const Tooltip = ({ province, mousePosition }) => {
    const tooltipRef = useRef(null);
    const [position, setPosition] = useState({ x: 0, y: 0 });

    // useLayoutEffect runs after render but before the browser paints, which is ideal for measurements to prevent flicker.
    useLayoutEffect(() => {
        if (province && tooltipRef.current) {
            ...
            // If the tooltip would go off the bottom edge of the screen, flip it above the cursor.
...
export default function HeatMapCard() {
    ...
    // We only need one mouse move listener on the main container.
    ...
    return (
        <>
            <div className="w-auto max-w-max bg-white p-4 rounded-2xl shadow-md" onMouseMove={handleMouseMove}>
                ...
                {/* This is still good practice to prevent the scaling from ever causing an issue. */}
            ...
            {/* The Tooltip is now passed the raw mousePosition */}
